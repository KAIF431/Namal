...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (logoLink) {
        logoLink.setAttribute("href", "namal.html");
    }
    ...
    // ==========================================
    const characterData = {
        faris: { name: "Faris Ghazi", desc: "Central Protagonist. Former prosecutor, brilliant strategist, seeking truth and justice." },
        saadi: { name: "Saadi Yousaf", desc: "Main Protagonist. Devout, intelligent, and loyal student who uncovers key clues." },
        zumar: { name: "Zumar Yousaf", desc: "Main Protagonist. Feisty & determined district attorney, seeking justice." },
        hashim: { name: "Hashim Kardar", desc: "Antagonist. High-profile corporate lawyer and formidable mastermind." },
        ahmar: { name: "Ahmar Shafi", desc: "Key Character. Skilled lawyer and key associate in the unraveling events." }
    };
    ...
    function autoHighlightCharacters() {
        ...
        storyBody.forEach(body => {
            ...
            paragraphs.forEach(p => {
                ...
                html = html.replace(/\bFaris Ghazi\b/g, '<span class="highlight-name" data-char="faris">Faris Ghazi</span>');
                html = html.replace(/\bFaris\b/g, '<span class="highlight-name" data-char="faris">Faris</span>');
                html = html.replace(/\bSaadi Yousaf\b/g, '<span class="highlight-name" data-char="saadi">Saadi Yousaf</span>');
                html = html.replace(/\bSaadi\b/g, '<span class="highlight-name" data-char="saadi">Saadi</span>');
                html = html.replace(/\bZumar Yousaf\b/g, '<span class="highlight-name" data-char="zumar">Zumar Yousaf</span>');
                html = html.replace(/\bZumar\b/g, '<span class="highlight-name" data-char="zumar">Zumar</span>');
                html = html.replace(/\bHashim Kardar\b/g, '<span class="highlight-name" data-char="hashim">Hashim Kardar</span>');
                html = html.replace(/\bHashim\b/g, '<span class="highlight-name" data-char="hashim">Hashim</span>');
    ...
    window.updateChapterStats = function() {
        ...
        const farisMatches = fullText.match(/\bFaris\b/gi);
        const saadiMatches = fullText.match(/\bSaadi\b/gi);
        const zumarMatches = fullText.match(/\bZumar\b/gi);

        const farisEl = document.getElementById('faris-mentions') || document.getElementById('salar-mentions');
        const saadiEl = document.getElementById('saadi-mentions') || document.getElementById('imama-mentions');
        const zumarEl = document.getElementById('zumar-mentions');

        if (farisEl) farisEl.innerText = farisMatches ? farisMatches.length : 0;
        if (saadiEl) saadiEl.innerText = saadiMatches ? saadiMatches.length : 0;
        if (zumarEl) zumarEl.innerText = zumarMatches ? zumarMatches.length : 0;
    };
...
document.addEventListener('click', (e) => {
    ...
    if (logoLink) {
        const href = logoLink.getAttribute('href') || 'namal.html';
...
document.addEventListener('selectionchange', () => {
    ...
    if (selectedText.length > 4 && selection.rangeCount > 0) {
        ...
        if (shareWa) {
            shareWa.onclick = (e) => {
                e.stopPropagation();
                const url = `https://api.whatsapp.com/send?text=${encodeURIComponent('"' + selectedText + '" — Namal Novel')}`;
        ...
        if (copyBtn) {
            copyBtn.onclick = (e) => {
                e.stopPropagation();
                navigator.clipboard.writeText(`"${selectedText}" — Namal Novel`);
                alert("Quote copied!");
